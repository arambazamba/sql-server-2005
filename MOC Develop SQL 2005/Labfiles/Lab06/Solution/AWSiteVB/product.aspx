...
<%@ import namespace="Microsoft.SqlServer.NotificationServices" %>
...
<script runat="server">
    ...
    Sub submitSubscription_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim inst As NSInstance
        Dim app As NSApplication
        Dim subdev As SubscriberDevice
        Dim subr As Subscriber
        Dim subn As Subscription

        inst = New NSInstance("AWReviewService")
        app = New NSApplication(inst, "AWReviews")

        ' 
        ' Create subscriber
        '
        subr = New Subscriber(inst)
        subr.SubscriberId = subscriberName.Text
        subr.Add()
   
        ' 
        ' Create an E-mail device for the subscriber
        '
        subdev = New SubscriberDevice(inst)
        subdev.DeviceTypeName = "Email"
        subdev.DeviceName = "myEmailDevice"
        subdev.DeliveryChannelName = "EmailChannel"
        subdev.SubscriberId = subscriberName.Text
        subdev.DeviceAddress = subscriberEmail.Text
        subdev.Add()

        ' 
        ' Create subscription
        '
        subn = New Subscription(app, "ProductReviewSubscription")
        subn.SetFieldValue("DeviceName", "myEmailDevice")
        subn.SetFieldValue("SubscriberLocale", "en-us")
        subn.SubscriberId = subscriberName.Text
        subn.SetFieldValue("ProductId", Integer.Parse(productID.Text))
        subn.Add()
           
        ' 
        ' Inform the user
        '
        subscriptionMessage.Text = "Thank you for your subscription"
        subscriberName.Enabled = False
        subscriberEmail.Enabled = False
        submitSubscription.Enabled = False
    End Sub
